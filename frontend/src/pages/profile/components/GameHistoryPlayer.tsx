...
export default function GameHistoryPlayer(props: GameHistoryPlayerProps) {
    ...
    return <div className="flex w-full h-5 items-center gap-5">
        <div className={classNames("rounded-full w-3 h-3", {
            "bg-white" : props.isWhite,
            "bg-slate-600": !props.isWhite
        })}>
        </div>
        {
            profile ?
            <div className="flex gap-5">
                ...
                <span className={classNames('text-base-content', {
                    "font-bold": props.isUser
                })}>{profile.displayName}{props.isUser ? " (You)" : ""}</span>
            </div>
            :
            <div className="flex gap-5 items-center">
                <div className="bg-base-200 w-5 h-5 rounded-full skeleton">
                </div>
