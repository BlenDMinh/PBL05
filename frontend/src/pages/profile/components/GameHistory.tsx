...
export default function GameHistory(props: GameHistoryProps) {
    ...
    if(!userId) {
        return <div className='w-full flex justify-center bg-base-300 rounded-lg p-5'>
                ...
                </tr>
                {
                    gameResults.map(res => 
                        <tr className="hover">
                            ...
                            <td className={classNames("text-lg", {
                                "text-success": (userId == res.player1Id && res.status == "PLAYER1_WIN") ||
                                (userId == res.player2Id && res.status == "PLAYER2_WIN"),
                                "text-error": !((userId == res.player1Id && res.status == "PLAYER1_WIN") ||
                                (userId == res.player2Id && res.status == "PLAYER2_WIN"))
                            })}>
                                {
                            ...
                            <td>
                                {new Date(res.createdAt + " UTC").toUTCString()}
                            </td>
                        </tr>
                    )
                }
